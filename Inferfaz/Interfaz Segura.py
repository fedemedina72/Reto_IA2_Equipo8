...
def subir_foto(foto, email, password):
    ...
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'path-a-.json'
    ...
    while cont:
        if foto:
            ...
            if len(faces) == 0:
                st.error(
                    'No se detectaron caras, favor de subir una foto correcta')
                cont = False
                return False
            else:
                user = auth.create_user_with_email_and_password(
                    email, password)
                # nombre del sub-folder donde se guardarán los datos (nombre de la persona)
                sub_data = datasets + '/' + user['localId'] + '/'
                ...
                for (x, y, w, h) in faces:
                    ...
                    # Convierte la imagen a un formato de bytes
                    _, buffer = cv2.imencode('.jpg', face_resize)
                    blob.upload_from_string(
                        buffer.tobytes(), content_type='image/jpg')
...
def signup(auth, email, password, db, foto):
    ...
    if submit:
        if '@tec.mx' not in email:  # Verificar email
            ...
            try:
                # obtener antes del @tec.mx
                mat = email.split('@tec.mx')[0].lower()
                ...
                if not (user == False):
                    ...
                    if 'a0' in mat:
                        ...
                        db.child(user['localId']).child("Rol").set('Docente')
...
if __name__ == '__main__':
    ...
    if user is not None:
        ...
        if rol == 'Docente':
            ...
            st.write(
                f"Click [aquí](<{url}>) para abrir la plataforma", unsafe_allow_html=True)
